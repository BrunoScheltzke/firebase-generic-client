...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        var bruno = Person(name: "Bruno", lastName: "Scheltzke")
                    ...
                    if case .success(_) = result {
                        PersonManager.shared.fetch(byId: bruno.firebaseId, completion: { (result) in
                            if case .success(let personFetched) = result {
                                DogManager.shared.ifNeeded(personFetched.dog, completion: { (result) in
                                    if case .success(let updatedDog) = result {
                                        if let dog = updatedDog {
                                            personFetched.dog.update(other: dog)
                                        }
